...
def zloz_zamowienie():
    ...
    for item in objects_lists.material_set: #szukam materialu o wybranym id w liscie materialow
        if item.id_number == wybrany_material:
            if objects_lists.user_set[logged_id].stan_konta >= item.cena: #sprawdzam czy stan konta jest wiekszy od ceny materialu
                if item.jest_dostepne: #sprawdzam czy material jest dostepny i zmniejszam dostepna liczbe o jeden
                    ...
                    objects_lists.user_set[logged_id].stan_konta -= item.cena #zmniejszam stan konta o cene materialu
...
def doladuj_konto():
    ...
    objects_lists.user_set[logged_id].stan_konta += kwota
